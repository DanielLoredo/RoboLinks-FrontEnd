...
import { link_tags } from "../../components/CreateLinkForm";
...
const SearchLinks = () => {
  ...
  const handleSearch = async (searchQueryText) => {
    ...
    //FILTER BY TAG
    if (link_tags.includes(searchQueryText)) {
      const tagFilter = { tags: [searchQueryText] };
      await getAllLinksApiRequest(tagFilter).then((response) => {
        const payload = { filter: 2, links: response.data };
        dispatch(getLinksByFilter(payload));
      });
    }
  };
